...
BEGIN
  ...
  -- Auto-trigger WhatsApp communication for certain status changes
  -- Only set if communication method isn't already explicitly set to something else
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    -- Send automatic WhatsApp when task is completed
    IF NEW.status = 'Complete' AND (NEW.communication_method IS NULL OR NEW.communication_method = '') THEN
    ...
    ELSIF NEW.status IN ('Waiting on Info', 'Sent Info', 'Call Arranged') AND 
          (NEW.communication_method IS NULL OR NEW.communication_method = '') THEN
      NEW.communicated = true;
      NEW.communication_method = 'WhatsApp';
    END IF;
  END IF;
